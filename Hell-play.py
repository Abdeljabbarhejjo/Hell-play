...
TEXT_FONT = pygame.font.Font("font.otf", 32)
...
background = pygame.transform.scale(pygame.image.load("desord.jpg"), WINDOW_SIZE)
...
def enemy_spawner():
    while True:
        ...
        enemy = Enemy(randomX, randomY, 75, 75, "enemy-Sheet.png", 2)
        player_center = player.get_center()
...
def spawn_particles(x, y):
    particle = Object(x, y, 75, 75, pygame.image.load("particles.png"))
...
# Objects
player = Player(WINDOW_SIZE[0] / 2, WINDOW_SIZE[1] / 2, 75, 75, "player-Sheettt.png", 5)
...
def display_ui():
    for i in range(player.max_health):
        img = pygame.image.load("heart_empty.png" if i >= player.health else "heart.png")
...
music = pygame.mixer.music.load("05 - Here's To The Pioneers.mp3")
...
while True:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_ESCAPE:  # التحقق من مفتاح "Esc"
                exit()  # الخروج من البرنامج عند الضغط على "Esc"
